\begin{table}[htbp]
  \centering
  \caption{Exact power of a CC~analysis when the two dropout probabilities differ and each group is inflated by its own dropout probability. Design parameters: $\pi_{1} = 0.45$, $\pi_{0} = 0.30$, $\rho_{1} = \rho_{0} = 0$, $r = 1$, $\alpha = 0.025$ (one-sided) and target power $= 0.8$. The target power is attained throughout, so equal dropout probabilities are not necessary for a CC~design to reach its target.}
