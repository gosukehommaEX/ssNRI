\begin{table}[htbp]
  ...
  \caption{Required sample sizes for the cytisine trial under the design assumptions of the original trial: $\pi_{1} = 0.47$, $\pi_{0} = 0.41$, $\omega_{1} = \omega_{0} = 0.10$, $r = 1$, $\alpha = 0.025$ (one-sided) and target power $= 0.8$. The simple inflation method gives $N_{\mathrm{CC}} = 2,388$, which is the sample size the trial enrolled.}
