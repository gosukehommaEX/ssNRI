...
test_that(".joint_cells keeps its names, including after clamping", {
  ...
  cells <- ssNRI:::.joint_cells(p = 0.4, omega = 0.15, rho = 0.1)
  ...
  for (p in c(0.2, 0.5, 0.8)) {
    for (omega in c(0.05, 0.2, 0.45)) {
      ...
      for (rho in c(b$rho_lower, 0, b$rho_upper)) {
        cells <- ssNRI:::.joint_cells(p = p, omega = omega, rho = rho)
        expect_named(cells, expected,
                     info = sprintf("p = %g, omega = %g, rho = %g",
                                    p, omega, rho))
...
test_that(".joint_cells reproduces the marginals and the correlation", {
  grid <- expand.grid(p = c(0.25, 0.50, 0.75), omega = c(0.05, 0.20, 0.40),
                      rho = c(-0.15, 0, 0.15))
  for (i in seq_len(nrow(grid))) {
    g <- grid[i, ]
    b <- rho_bounds(p = g$p, omega = g$omega)
    if (g$rho < b$rho_lower || g$rho > b$rho_upper) next
    cells <- ssNRI:::.joint_cells(p = g$p, omega = g$omega, rho = g$rho)
    info <- sprintf("p = %g, omega = %g, rho = %g", g$p, g$omega, g$rho)
    expect_equal(sum(cells), 1, tolerance = 1e-12, info = info)
    expect_equal(cells[["pi_10"]] + cells[["pi_11"]], g$p,
                 tolerance = 1e-12, info = info)
    expect_equal(cells[["pi_11"]] + cells[["pi_01"]], g$omega,
                 tolerance = 1e-12, info = info)
    back <- (cells[["pi_11"]] - g$p * g$omega) /
      sqrt(g$p * (1 - g$p) * g$omega * (1 - g$omega))
    expect_equal(back, g$rho, tolerance = 1e-10, info = info)
  }
})

# Deliberately naive threshold: scan x1 upwards until the test rejects
brute_threshold <- function(m1, m2, z_alpha) {
  vapply(0:m2, function(j) {
    for (u in 0:m1) {
      pp <- (u + j) / (m1 + m2)
      se <- max(sqrt(pp * (1 - pp) * (1 / m1 + 1 / m2)), 1e-10)
      if ((u / m1 - j / m2) / se > z_alpha) return(u)
    }
    m1 + 1
  }, numeric(1))
}
...
test_that(".reject_threshold matches a direct scan", {
  ...
  for (m1 in c(1, 2, 5, 17, 40, 63)) {
    for (m2 in c(1, 3, 8, 23, 55)) {
      fast <- ssNRI:::.reject_threshold(m1, m2, z)
      expect_equal(length(fast), m2 + 1L,
                   info = sprintf("m1 = %d, m2 = %d", m1, m2))
      expect_equal(fast, brute_threshold(m1, m2, z),
                   info = sprintf("m1 = %d, m2 = %d", m1, m2))
    }
  }
})
...
test_that(".reject_threshold survives group sizes past the integer limit", {
  ...
  # arithmetic that overflows from m1 = m2 = 64 upwards, which used to produce
...
test_that(".reject_threshold is non-decreasing in the group 2 count", {
  z <- stats::qnorm(0.975)
  for (m1 in c(7, 31, 80)) {
    for (m2 in c(5, 29, 77)) {
      cth <- ssNRI:::.reject_threshold(m1, m2, z)
...
test_that(".common_rho_range intersects the two feasible ranges", {
  rng <- ssNRI:::.common_rho_range(p1 = 0.6, p2 = 0.4,
                                   omega1 = 0.2, omega2 = 0.2)
  b1 <- rho_bounds(p = 0.6, omega = 0.2)
  b2 <- rho_bounds(p = 0.4, omega = 0.2)
  expect_equal(rng[1], max(b1$rho_lower, b2$rho_lower))
  expect_equal(rng[2], min(b1$rho_upper, b2$rho_upper))
})

test_that(".clamp_rho moves a value into the feasible range and leaves others", {
  b <- rho_bounds(p = 0.3, omega = 0.1)
