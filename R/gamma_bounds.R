...
#' containing \code{pi}, \code{omega}, \code{gamma_lower}, \code{gamma_upper}
#' and the equivalent correlation bounds \code{rho_lower} and \code{rho_upper}.
#'
#' @details
#' With gamma = Pr(R = 1 | D = 1) the four joint cells are
#' gamma omega, (1 - gamma) omega, pi - gamma omega and 1 - pi - omega +
...
#' the two endpoints. The upper bound changes branch at omega = pi and the lower
#' bound at omega = 1 - pi, exactly where the correlation bounds do.
#'
...
#'
#' @examples
#' gamma_bounds(pi = 0.45, omega = 0.10)
#' gamma_bounds(pi = 0.20, omega = 0.30)
#'
#' @seealso \code{\link{rho_bounds}}, \code{\link{rho_to_gamma}},
#' \code{\link{gamma_to_rho}}
#'
#' @export
gamma_bounds <- function(pi, omega) {

  # Input validation
  if (length(pi) != 1 || length(omega) != 1) {
    stop("pi and omega must each be of length one")
  }
  ...
  gamma_lower <- max(0, (pi + omega - 1) / omega)
  gamma_upper <- min(1, pi / omega)
  ...
  result <- data.frame(
    ...
    rho_lower   = gamma_to_rho(pi, omega, gamma_lower),
    rho_upper   = gamma_to_rho(pi, omega, gamma_upper)
  )
...
print.gamma_bounds <- function(x, ...) {
  ...
  cat("Feasible range for gamma = Pr(R_ij = 1 | D_ij = 1):\n")
...
#' restriction binds only once the dropout probability exceeds pi or 1 - pi. What
...
#'
#' @examples
#' plot(gamma_bounds(pi = 0.45, omega = 0.10), n_points = 21)
#'
...
plot.gamma_bounds <- function(x, omega_values = NULL, n_points = 101, ...) {
  ...
  dat <- do.call(rbind, lapply(omega_values, function(w) {
    b <- rho_bounds(pi = x$pi, omega = w)
    rho <- seq(b$rho_lower, b$rho_upper, length.out = n_points)
    data.frame(
      omega = w,
      rho = rho,
      gamma = pmin(pmax(rho_to_gamma(pi = x$pi, omega = w, rho = rho), 0), 1),
  ...
  ggplot2::ggplot(
    ...
    ggplot2::geom_hline(yintercept = x$pi, linetype = "dotted") +
    ...
    ggplot2::labs(
      ...
      title = parse(text = sprintf(
        'paste("Correlation and dropout response probability at ", pi, " = %.2f")',
        x$pi
      ))
    ) +
